...
namespace AdventOfCode.Year2021
{
    ...
    {
        // Puzzle can be found on: https://adventofcode.com/2021/day/7

        /**
         * Key Insights
         * 1: check the steps that needs to be taken; not to determine an equilibrium.
         * 2: Bruteforce works fine
         * 3: Recognize Triangle Numbers, and have O(o) calculation on that
         */

        public string SolvePart1(string input)
        {
            // Convert input to something we can understand
            int[] crabPositions = Array.ConvertAll(input.Split(',', StringSplitOptions.RemoveEmptyEntries), x => int.Parse(x));
            ...
            while (currentPosition <= max)
            {
                ...
                foreach (int i in crabPositions)
                {
                    currentFuel += Math.Abs(currentPosition - i);
                }
                // replace the minimum amount if this round was using less
                fuelUsed = fuelUsed < currentFuel ? fuelUsed : currentFuel;
                currentPosition++;
            }
        ...
        public string SolvePart2(string input)
        {
            ...
            // no need for pre-calc -> These are triangle numbers 0 = 0
            // 0 steps = 0
            // 1 steps = 1
            // 2 steps = 1 + 2 = 3
            // 3 steps = 3 + 3 = 6
            ...
            // Otherwise precalculate the increasing demand of fuelcosts
            //int[] fuelCosts = new int[max];
            //for (int i = 1; i < max; i++)
            //{
            ...
            while (currentPosition <= max)
            {
                ...
                {
                    // steps 
                    int steps = (Math.Abs(currentPosition - i) );
