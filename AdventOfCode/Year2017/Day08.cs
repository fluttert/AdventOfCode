...
namespace AdventOfCode.Year2017
{
    public class Day08 : IAoC
    {
        public string SolvePart1(string input)
        {
            ...
            foreach (string instuction in instructions)
            {
                // break down instruction
                var parts = instuction.Split(' ');
                string reg1 = parts[0];
                string op1 = parts[1];
                int num1 = int.Parse(parts[2]);
                string reg2 = parts[4];
                string op2 = parts[5];
                ...
                // check the if clause
                bool executeFirstPart = false;
                switch (op2)
                {
                    case "<":
                        executeFirstPart = registers[reg2] < num2;
                        break;
            ...
            return registers.Max(e => e.Value).ToString();
        }
        ...
        public string SolvePart2(string input)
        {
            var registers = new Dictionary<string, int>();
            string[] instructions = input.Split(Environment.NewLine);
            int highestEverValue = int.MinValue;
            ...
            foreach (string instuction in instructions)
            {
                ...
                if (executeFirstPart)
                {
                    switch (op1)
                    {
                        case "inc":
                            registers[reg1] += num1;
                            if (registers[reg1] > highestEverValue) { highestEverValue = registers[reg1]; }
                            break;
            ...
            // return maximum value
            return highestEverValue.ToString();
        }

        public string GetInput() => @"aj dec -520 if icd < 9
z dec -500 if b <= 2
zz dec 628 if z >= 499
db dec -818 if u >= 0
